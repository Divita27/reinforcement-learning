...
class VortexENV(gym.Env):
    ...
    def reset(self, start_time="random"):
        ...
        elif self.agent_type == 'long_sighted_agent':
            observation = (rel_goal_x/self.WIDTH, rel_goal_y/self.HEIGHT, bg_vel_x, bg_vel_y) + tuple(self.get_custom_bg_velocity())
        else:
    ...
    def step(self, action):
        ...
        bg_vel_x = self.ux[min(int(self.y), 99), min(int(self.x), 399)]
        bg_vel_y = self.uy[min(int(self.y), 99), min(int(self.x), 399)]
        ...
        current_vorticity = current_vorticity_matrix[min(int(self.y), 99), min(int(self.x), 399)]
    ...
    def get_custom_bg_velocity(self):
        
        len_bg_sample = 8 # 15 incase of 3x3 grid
        units = len_bg_sample//2 # /3 in case of 3x3 grid

        # limit units wrt to the grid size
        if int(self.y)-int(units) < 0 or int(self.y)+int(units) > 99 or int(self.x)-int(units) < 0 or int(self.x)+int(units) > 399:
            return [0] * len_bg_sample
        
        # bg for 3x3 grid
        # bg_vel_x = self.ux[int(self.y)-math.floor((3/2)*units): int(self.y)+math.ceil((3/2)*units), int(self.x)-math.floor((3/2)*units): int(self.x)+math.ceil((3/2)*units)]
        # bg_vel_y = self.uy[int(self.y)-math.floor((3/2)*units): int(self.y)+math.ceil((3/2)*units), int(self.x)-math.floor((3/2)*units): int(self.x)+math.ceil((3/2)*units)]
        
        # avg_bg_vel_x = np.zeros((3, 3))
        # avg_bg_vel_y = np.zeros((3, 3))

        # for i in range(3):
        #     for j in range(3):
        #         avg_bg_vel_x[i, j] = np.mean(bg_vel_x[i*units: (i+1)*units, j*units: (j+1)*units])
        #         avg_bg_vel_y[i, j] = np.mean(bg_vel_y[i*units: (i+1)*units, j*units: (j+1)*units])

        # center of the grid vel
        # bg_center_grid_x = bg_vel_x[units : 2*units, units : 2*units]
        # bg_center_grid_y = bg_vel_y[units : 2*units, units : 2*units]


        # bg for 2x2 grid
        bg_vel_x = self.ux[int(self.y)-int(units): int(self.y)+int(units), int(self.x)-int(units): int(self.x)+int(units)]
        bg_vel_y = self.uy[int(self.y)-int(units): int(self.y)+int(units), int(self.x)-int(units): int(self.x)+int(units)]

        avg_bg_vel_x = np.zeros((2, 2))
        avg_bg_vel_y = np.zeros((2, 2))
        ...
        # avg background velocity for 8 surrounding grids + the center grid velocity
        # return list(avg_bg_vel_x.flatten()) + list(avg_bg_vel_y.flatten()) + list(bg_center_grid_x.flatten()) + list(bg_center_grid_y.flatten())

        # 4 surrounding grids x 2 for ux and uy = 8
        return list(avg_bg_vel_x.flatten()) + list(avg_bg_vel_y.flatten())
